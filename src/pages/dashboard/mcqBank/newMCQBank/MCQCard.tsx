...
const MCQCard: React.FC<MCQCardProps> = ({
  ...
  const filteredOptions = (question?.options || []).filter(
    (opt) => opt && opt.optionText && String(opt.optionText).trim().length > 0
  );
  ...
  return (
    <div className="border border-slate-300 rounded-lg p-5 space-y-4 bg-white">
      ...
      <div className="space-y-2">
        {filteredOptions.map((opt, optionIdx) => {
      ...
      {showAnswer && (
        <div className="mt-4 p-4 bg-slate-100 rounded-lg">
          ...
          {(() => {
            const correctOpt = (question?.options || []).find((o) => o?.option === question?.correctOption && o?.optionText && String(o.optionText).trim().length > 0);
