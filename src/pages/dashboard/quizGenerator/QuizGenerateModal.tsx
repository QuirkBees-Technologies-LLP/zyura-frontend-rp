...
const prevQuizMode = useRef(quizMode);
...
useEffect(() => {
  ...
  } else if (quizMode === "exam" && !examName) {
    setQuestionCount(0);
    setDuration(0);
  } else if (quizMode === "study" && prevQuizMode.current !== "study") {
    // Only reset when switching INTO study mode, not on every render
    setQuestionCount(5);
    setDuration(10);
  }

  prevQuizMode.current = quizMode;
  ...
  const handleSubmit = async () => {
    ...
    if (quizMode === "exam") {
      ...
      navigate(`/dashboard/quiz/${examName}?${examQuery.toString()}`);
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        ...
        setOpen(false);
        navigate(`/dashboard/quiz/${quizId}?duration=${duration * 60}&limit=${questionCount}`);
      }
    } catch (error: any) {
      ...
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto thin-scrollbar">
        ...
        <DialogFooter>
          ...
          <Button
            ...
            className="bg-brand text-white hover:bg-brand/90 transition-all"
          >
            {isGenerating ? (
