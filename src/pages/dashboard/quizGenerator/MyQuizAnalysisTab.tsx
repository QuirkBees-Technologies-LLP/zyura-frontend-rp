...
} from "@/components/quizOverview/normalizeRecommendations";
...
const MyQuizAnalysisTab: React.FC = () => {
  ...
  const isExamMode =
    queryParams.get("source") === "exam" || location.state?.isExamMode;
  ...
  const { data: examResponse, isLoading: examLoading } = useGetSingleExamQuery(
    { id: id || "" },
    {
      skip: !id || !isExamMode,
    },
  ...
  const [isAILoading, setIsAILoading] = useState(
    !!location.state?.isGeneratingRecommendation && !isExamMode,
  );

  // Stop loading if we have content OR if we finished a fetch and gpt might have returned nothing
  useEffect(() => {
    if (!isFetching && response) {
      const raw = quizData?.tracking?.recommendedContent;
      const recommendations = normalizeRecommendations(raw ?? {}, id || "");
      if (recommendations && hasAnyRecommendationContent(recommendations)) {
        setIsAILoading(false);
      }
    }
  }, [isFetching, response, quizData, id]);
  ...
  if (id) {
    const storedTracking = sessionStorage.getItem(`quiz_tracking_${id}`);

    if (storedTracking) {
      try {
        savedTracking = JSON.parse(storedTracking);
      } catch (error) {
        console.error("Failed to parse saved tracking", error);
      }
    }
  }

  const actualTracking =
    trackingDataFromState ||
    tracking ||
    (savedTracking &&
    typeof savedTracking === "object" &&
    Object.keys(savedTracking).length > 0
      ? savedTracking
      : {}) ||
    {};
  ...
  const correctPercentage =
    actualTracking?.totalAttemptCount > 0
      ? Math.round(
          (actualTracking?.correctMcqCount /
            actualTracking?.totalAttemptCount) *
            100,
        )
      : 0;
      ...
      <div className="py-6">
        <div className="w-full space-y-6">
          ...
          <div className="flex items-start gap-1">
            <Link
              to={isExamMode ? "/dashboard/mcq-bank" : "/dashboard/quiz-page"}
              className="sm:mb-0"
            >
            ...
            <div className="flex items-center gap-3 p-4 bg-blue-50 border border-blue-200 rounded-xl animate-pulse">
              ...
              <div>
                ...
                <p className="text-xs text-blue-700">
                  Analyzing your weak areas to generate personalized
                  recommendations...
                </p>
              </div>
            </div>
          ...
          >
            <div
              className={`bg-white p-2 mb-11 rounded-xl shadow-sm border border-gray-100 ${isExamMode ? "w-full max-w-2xl mx-auto" : ""}`}
            >
