...
export default function AllNotesTab({ notes, loading }: any) {
  // const [incrementDownloadCount] = useIncrementNoteDownloadCountMutation();    -----previously used for download count increment, now handled in backend via download URL transformation
    ...
    <NoteCard
      ...
      date={new Date(note.createdAt).toLocaleDateString("en-GB", {
        ...
        year: "numeric",
              ...
              isFirstRow={index < 3}
      onClick={() => {
        const pdfUrl = note.notes?.[0]?.fileUrl;
        if (pdfUrl) handleViewPdf(pdfUrl);
      }}
    />
  ))}
</div>
      ...
      {selectedPdf && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center">
          <div className="w-[95%] h-[95%] bg-white rounded-xl overflow-hidden flex flex-col relative">
            {/* HEADER */}
