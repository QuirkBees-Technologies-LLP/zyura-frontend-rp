...
const DashboardSearch: FC<DashboardSearchProps> = ({
  ...
  const searchableSidebarItems = useMemo(() => {
    return role === "STUDENT" || role === "PROFESSIONAL"
  ...
  useEffect(() => {
    if (value.trim().length > 0) {
      const filtered = searchableSidebarItems.filter((item) =>
  ...
  return (
    <div className="relative w-full max-w-sm" ref={dropdownRef}>
      <div
        className={cn(
          "flex items-center w-full gap-1 border border-border rounded-md bg-gray-100 px-3 transition-shadow duration-200 focus-within:ring-2 focus-within:ring-primary/20",
          className,
        )}
      >
