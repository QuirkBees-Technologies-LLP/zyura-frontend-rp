...
import { isMyContentRepository } from "@/utils/studyPlanNavigation";
...
const SmartStudyPlan: React.FC = () => {
  ...
  const handleStartClick = (task: any) => {
    ...
    const repo = task.suggest_content?.contentRepository;
    ...
    const navigationState = {
      ...
      from: "home",
      planLimit: task.suggest_content?.limit,
      contentRepository: repo ?? "bank",
    };
    ...
    if (taskType === "mcqs" || taskType === "mcq") {
      ...
      navigate(`/dashboard/solve-flash-card/${contentId}`, {
        state: {
          ...navigationState,
          source: isMyContentRepository(repo) ? "generated" : undefined,
        },
      });
    } else if (
