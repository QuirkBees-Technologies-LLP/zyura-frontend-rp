...
const CreateDiscussion = ({ onBack }: CreateDiscussionProps) => {
  ...
  const onSubmit = async (values: FormData) => {
    ...
    try {
      ...
      console.log("Forum post created successfully:", payload);


      toast.success("Discussion posted to the forum.");
      onBack();
    } catch (error: any) {
      console.error("Error creating forum post:", error);
      toast.error(
        error?.data?.message || "Could not create this discussion. Try again.",
      );
    }
  };
              ...
              >
                <SelectTrigger
                  className={
                    errors.category
                      ? "border-red-500"
                      : isTreeLoading || subjects.length === 0
