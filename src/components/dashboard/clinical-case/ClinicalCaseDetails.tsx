...
const ClinicalCaseDetails: React.FC<CaseDetailProps> = ({ onBack }) => {
  ...
  const clinicalCase = (stateClinicalCaseData ||
    (isGenerated
      ? generatedData?.data
      : standardData?.data)) as ClinicalCaseData;
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-slate-500">
          The clinical case you are looking for does not exist or has been
          removed.
        </p>
        <Link to="/dashboard/clinical-case-generator">
          <PrimaryButton className="bg-brand hover:bg-blue-main/90">
            Back to Cases
          </PrimaryButton>
        </Link>
      </div>
                ...
                <div className="flex items-center gap-4">
                  <button
                    onClick={() => {
                      ...
                      if (
                        location.state?.fromRecommendedBundle &&
                        quizId
                      ) {
                        navigate(
                          `/dashboard/clinical-case/recommended-bundle/${quizId}`,
                          {
                            state: {
                              fromAnalysis: true,
                              quizId,
                      ...
                      } else if (fromAnalysis && quizId) {
                        navigate(`/dashboard/quiz-analysis/${quizId}`);
                      } else if (
                        location.state?.from === "weekly-plan" &&
                        planId
                      ) {
                        navigate(`/dashboard/weekly-plan/${planId}`); // ← goes back to exact plan
                      } else if (location.state?.from === "weekly-plan") {
                        navigate(-1); // fallback if no planId
