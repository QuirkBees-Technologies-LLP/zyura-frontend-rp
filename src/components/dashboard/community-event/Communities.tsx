...
const Communities = ({ activeTab: externalActiveTab, onTabChange }: CommunitiesProps) => {
  ...
  const handleCardClick = (tabName: string) => {
    ...
    navigate("/dashboard/all-communities", { state: { activeTab: tabName } });
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {cardConfigs.map((config, index) => (
          <CommunityEventHomeCard
            key={index}
            {...config}
            onClick={() => handleCardClick(config.tabName)}
          />
        ))}
      </div>
    </div>
